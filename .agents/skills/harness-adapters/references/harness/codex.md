...
| Autonomy | `--approve-for-me` plus `-c sandbox_workspace_write.network_access=true` and `--add-dir` for the status record parent, this task's inbox and brief/report directory, and `$HOME/tmp`, omitting absent directories and deduplicating resolved paths. `--approve-for-me` auto-reviews under the workspace-write sandbox (`codex --help` on 0.154.0); it does not disable the sandbox the way `--dangerously-bypass-approvals-and-sandbox` did. The 2026-09-12 codex-cli 0.154.0 probe used the earlier whole-home grant; live proof of the narrowed grants remains outstanding in `docs/verification/runtime-backends.md`. |
