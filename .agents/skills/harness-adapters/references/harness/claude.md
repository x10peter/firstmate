...
| Permissions | `--permission-mode auto` by default, or `--dangerously-skip-permissions` when `config/claude-permission-mode` is `bypass`; the `auto` shape verified on 2.1.269, outside-cwd writes and GitHub reach verified on 2.1.269 (2026-09-12), and `../../../../../docs/configuration.md` "Claude permission mode" owns the file. |
...
Neither `--permission-mode auto` nor `--dangerously-skip-permissions` covers that gate: `claude --help` records that the dialog is skipped only in non-interactive mode, through `-p` or a non-TTY stdout, and a spawned pane is interactive.
...
A default auto launch never meets the bypass confirmation, because it does not request bypass mode: on 2.1.269 `claude --permission-mode auto` reached the composer directly with the footer `⏵⏵ auto mode on (shift+tab to cycle)`.
